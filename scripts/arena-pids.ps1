...
try {
  $dataRoot = if ($env:ARENA_DATA_ROOT) { $env:ARENA_DATA_ROOT } else { Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) 'data' }
  $dataRootPattern = [regex]::Escape("--data-root=$dataRoot")
