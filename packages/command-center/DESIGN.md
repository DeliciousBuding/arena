# Arena 指挥面板 Design System（群星式深空指挥台 · Stellaris Deck）

> 本文件是前端视觉体系的唯一事实源（SSOT）。改样式前先读这里；改这里后要同步
> `public/style.css` 的 `:root` token 与组件实现。
> 设计母题：Paradox《群星 Stellaris》界面语言——深空蓝黑底、金色/琥珀强调、
> 冷蓝细亮线描边、面板顶部微光、帝国总览式顶栏资源条、outliner 式舰队索引。

## 1. 设计原则

1. **深空蓝黑基调**：界面以深空蓝黑（`#04060a`~`#0a0e16`）为底，彩色用于
   「数据/状态/身份」三类语义 + 群星式金/青强调，不做花哨装饰渐变。
2. **细亮线描边**：卡片/面板用冷蓝灰细亮线 `rgba(140,168,210,.14-.28)` 表达层级，
   hover 提亮 + 微光；阴影辅助抬升。
3. **金色主强调**：主行动色 = 群星 subtitle 金 `#e4a02e`（primary 按钮金底深字）；
   hover 走金色提亮；青色 `#1fe0ca` 为科技/数据强调（hover/active/焦点）。
4. **面板顶部微光条**：每张面板顶部 1px 金色渐变光条（`::before`），群星 panel 特征。
5. **帝国总览**：顶栏 = 每租户 资源/工人/增量 数据格（empire-cell），群星 topbar 资源条
   同构；舰队索引 outliner 化（金色选中态 + 人类指挥 H 徽章）。
...
| Token | 值 | 用途 |
|---|---|---|
| `--bg-deep` / `--bg` | `#04060a` / `#07090f` | 页面底 / 次级底（深空蓝黑） |
| `--surface` | `rgba(11,15,24,.78)` | 面板底（+blur） |
| `--surface-solid` / `--surface-raised` | `#0a0e16` / `rgba(18,24,36,.92)` | 实底 / 悬浮 |
| `--border` / `--border-strong` / `--border-faint` | `rgba(140,168,210,.14/.28/.08)` | 三档冷蓝细亮线 |
| `--text` / `--text-dim` / `--text-faint` | `#eef2f8` / `#9fb0c4` / `#5f6f85` | 三档文字（米白/蓝灰） |
| `--accent` | `#e4a02e` | 金色主强调（群星 subtitle） |
| `--success` / `--warn` / `--danger` | `#87cf9e` / `#e4a02e` / `#fc5646` | 数据/状态语义 |
| `--green-resource` / `--cyan-signal` | `#7fd8a5` / `#1fe0ca` | 资源绿 / 科技青（群星 cyan） |
| `--t1..--t4` | 蓝/绿/紫/红（muted） | 租户身份（地图/小色块） |

语义色规则：金色 = 强调/主行动/指挥中；青色 = 科技/数据 hover；绿 = 资源/正向；
红 = 危险/敌意。租户色只出现在「地图 canvas、租户名小色块、流内租户列」——
**禁止**整卡染色/左侧竖条。
...
| Token | 值 | 用途 |
|---|---|---|
| `--radius-sm/md/lg` | 5/8/12px | 小件 / 控件 / 卡片面板（仅 3 档，偏锐利） |
| `--shadow-card` | `0 0 0 1px rgba(120,160,220,.05), 0 8px 24px rgba(0,0,0,.42)` | 卡片 |
| `--shadow-float` | `0 24px 64px rgba(0,0,0,.66), 0 0 0 1px rgba(150,185,235,.08)` | 悬浮层/弹窗 |
...
- **按钮**：默认 = 冷蓝细亮线 + 5% 蓝底；primary = 金色渐变底深字（群星 subtitle）；
  ghost = 透明；active 统一 `scale(.97)`；focus-visible 青色 outline。
- **租户卡**：无左侧色条；租户身份 = 名字前 8px 圆角小色块（`.tenant-name::before`）；
  solo = 租户色 40% 透明度 1px 环 + 6% 极淡底色 + 顶部金线；卡片紧凑（padding 9×12、指标 12.5px）。
- **帝国总览（顶栏资源条）**：`.empire-strip` = 每租户一数据格（`.empire-cell`，T1 蓝 / T2 绿 /
  T3 紫 / T4 红 租户色标签 + 资源值 + 工人·增量小字），hover 租户色描边提亮——群星 topbar 资源条同构。
- **侧栏顺序（首屏优先）**：租户卡 → 图例 → 图层 → 租户视图 →（单租户专属）舰队HUD → 舰队索引；
  单租户专属面板放底部，避免图例/图层被挤出首屏。
- **面板**：深空蓝黑渐变底 + blur(16) + 冷蓝细亮线 + 顶部 1px 金色微光条（`::before`）；
  hover 提亮边框。标题 = 金色大写等宽（`.panel-title`，群星 eyebrow 同款）。
- **标签页**：白 1.5px 下划线（scaleX 动画），激活文字纯白。
- **舰队索引（outliner）**：`.asset-row` hover 蓝底提亮；`.active` = 金色渐变底 + 金边 + 微光
  （群星选中舰船同款）；人类指挥中单位 = 金色描边 + H 徽章（`.asset-h`）。
...
- **商店面板（右栏）**：eyebrow 小标题（`OFFICIAL STORE · LINUXDO`）+ 15px 主标题层级；分区
  ...
  限购），hover 上浮 1px + 阴影。
- **库存徽章（`available_stock` 三档语义色）**：`库存 N` = success 描边/底色；`仅剩 ≤5` = warn（金）；
...
- **人类指挥中标记（2026-08-08）**：舰队索引行琥珀描边 + H 徽章（`.asset-h`），地图受控单位画琥珀色虚线环 + 头部 H 标签（`drawHumanMarker`）——指挥官一眼看出哪些单位已被人工接管（琥珀 = 指挥中状态语义，不撞 4 租户色）。
...
- **右键指挥菜单（2026-08-08，群星 Context Command Menu）**：画布 `contextmenu` = 命中
  单位/核心弹命令菜单（`.ctx-menu`：深空底 + 金顶微光 + 单位头 + 命令列），右键空白 =
  取消选中（群星右键空白收镜语义）。菜单项与动作面板同源（`tactActionTypes` +
  `tactAvailability`）：可用项青 hover，不可用项金色 blocked 带原因（点击提示 shake）；
  SELF_DESTRUCT/WAIT 不入菜单（危险/无意义动作只在动作面板）。点击菜单项复用
  `tactChooseAction`（一键提交或进入选点模式）；Esc/✕/空白关闭。
- **Outliner 分组（2026-08-08，群星资产列表）**：舰队索引按 核心/工人/先锋/游侠 分组
  （`.asset-group`：组头 = 折叠箭头 + 大写组名 + 金色计数徽章），组可折叠并持久化
  （`arena-cc.assetGroups`）；行内含 HP 微条（`.asset-hpbar`，低血红渐变），核心以
  hp/shield 当前最大值为基准满条。选中行 = 金色渐变底 + 金边（群星选中舰船同款）。
- **快捷键指挥（2026-08-08，群星式）**：选中单位后 M=移动（核心=开始迁移方向）、
...
- **参谋建议面板（右栏，main 合入 2026-08-08）**：`STAFF ADVICE`——联盟态势/共享测绘/
  ...
  四档 severity 胶囊：CRITICAL=红、HIGH=金、MEDIUM=蓝、INFO=灰（与威胁情报 tier 同语言）；
...
- **数据管线健康芯片（顶栏，main 合入 2026-08-08）**：`health-chip` 三态胶囊——
  测绘同步（绿）/ 测绘滞后 Nt（金）/ 测绘缺失（红，列出缺失租户），hover 显示
  最大/平均滞后 tick；/api/health/pipeline 15s 轮询。
- **事迹徽章（决策流，main 合入 2026-08-08）**：`.st-badge.deed`（金）/ `.deed-hot`（红）
...
- **威胁扇区玫瑰（2026-08-08，全局联盟）**：每己方核心按 8 方向画敌情扇区条
  ...
  <32 格显示最近距离（黄）、<18 红；随「敌情热区」图层开关显隐——大地图一眼看出
  "哪个方向敌核在逼近我方哪颗核心"。
