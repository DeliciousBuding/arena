...
test("mine-trend: 首采后缺口缩小", () => {
  const resources = [
    ...
    { cell: "2,2", firstSeenTick: 200, lastSeenTick: 9000 },  // 一直可见，从未采集
    { cell: "3,3", firstSeenTick: 100, lastSeenTick: 5000 },  // tick5000 后不再可见
  ];
  const events = [
  ...
  // 窗口 0 (endTick 6000, cutoff 4000)：1,1 已采 / 2,2 未采 / 3,3 lastSeen 5000>=4000 可见未采
  assert.equal(tr.trend[0]?.visible, 3, "1,1 + 2,2 + 3,3 可见");
  assert.equal(tr.trend[0]?.visibleNever, 2, "2,2 + 3,3 未采");
  // 窗口 2 (endTick 10000, cutoff 8000)：3,3 lastSeen 5000 < 8000 → 不可见
...
test("mine-trend: 空输入兜底 + 全采集", () => {
  ...
  const resources = [{ cell: "9,9", firstSeenTick: 100, lastSeenTick: 8000 }];
