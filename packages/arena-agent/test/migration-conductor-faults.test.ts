...
test("故障2：lease 过期 → isMigrationLeaseFresh=false；step 拒绝续迁（计划原样保留、held 不变、无转移）", () => {
  ...
  const held = { holdEntryCount: 1, holdFirstTick: 5_500, holdTicks: 2, settleElapsed: 0, stallTicks: 0, clearRetries: 0, threatStallTicks: 0, threatFirstTick: 0, threatReplanCount: 0, gapTicks: 0 };
  const result = conductorStep({
    tick: expiredTick,
    nowMs: NOW_BASE_MS,
