...
test("军事产兵：首选 RANGER 买不起（res 13 < 14）→ 回退产 VANGUARD（t2 冻结回归）", () => {
  ...
  assert.deepEqual(r.action, { type: "SPAWN", unitType: "VANGUARD" }, "应回退产 VANGUARD 打破冻结");
  assert.equal(r.intent, "spawn_vanguard_military_ratio");
});

test("军事产兵：首选买得起仍产首选（配比语义不变）", () => {
  // 3V+0R、res 15 ≥ RANGER 12+2=14 → 首选 RANGER
  const state = makeState(12, 3, 0, 15);
  const r = selectDeterministicCoreAction(state, null, POLICY, 0.5, 30, false, 2, 20, false);
  assert.deepEqual(r.action, { type: "SPAWN", unitType: "RANGER" });
  assert.equal(r.intent, "spawn_ranger_military_ratio");
});
