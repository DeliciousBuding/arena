/** route-aware 任务分配测试（生产回流 99b4ba2/d2fe2f6 契约）：
 *  shortestPathDistances 距离场 + WorkerTaskPlanner 障碍感知代价 +
 *  DeterministicPlanner 记忆矿单一分配权威。 */
...
test("multi-target BFS: no obstacles falls back to manhattan (bounded)", () => {
  const distances = shortestPathDistances([0, 0], [[5, 0], [0, 7]], new Set());
  assert.equal(distances.get("5,0"), 5);
  assert.equal(distances.get("0,7"), 7);
});
...
test("WorkerTaskPlanner: enemy-occupied resource cell is ineligible even if nearest", () => {
  const plan = new WorkerTaskPlanner().plan({
    ...snapshot([["1,0", { position: [1, 0], visible: true, lastSeenTick: 100 }]]),
    enemyCells: new Set(["1,0"]),
  });
  assert.equal(plan.assignments[0]!.task.type, "WAIT");
});
