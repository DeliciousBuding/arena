...
export class SafetyPlanner {
  ...
  decide(input: SafetyPlannerInput): Plan {
    ...
    // 预测断链保持：敌方被锁原地后 prevPosition==position → 差分消失 →
    // enemyReturnPath 无预测 → 若清空配对锁手散伙、敌方原地重试成功突破。
    // 断链时保留上一 tick 配对，锁手继续守原锁点（锁龄超限由 decideWorker
    // 的 lockMaxTicks 兜底释放）。
  ...
  private decideWorker(
    ...
    if (unit.cargo > 0) {
      ...
      // 移入核心格、38 次已在核心格）。产兵价值 > 1 资源卸货，让位净赚。
      // 已在核心格 → 让出（复用 coreClearance 出口）；在邻格 → WAIT 不进核心
      // 格（下 tick 核心 spawn 完成后正常卸货）。
