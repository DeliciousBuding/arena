...
export class SafetyPlanner {
   ...
   *
   *  评分元组 (-onward_open, enemyCount, directionRank) 取最小值——onward_open
   *  越高越好（更多逃生通道），enemyCount 越低越好（安全），directionRank
   *  固定（UP=0,RIGHT=1,DOWN=2,LEFT=3，与 homeCell 同序）。
   *
   *  候选筛选：非障碍 + 非敌占 + occupancy < 2（容量未满）。onward_open 不含
   *  Core 自身位置（避免把"回核心"算作开放通道）。全部满/被堵 → fallback
   *  到 coreGuardFallback（cheb-2 外圈）。 */
  private egressExit(
    home: Position,
    obstacles: ReadonlySet<string>,
    occupancy: ReadonlyMap<string, number>,
    ...
    const cardinals: readonly { readonly delta: readonly [number, number]; readonly rank: number }[] = [
      { delta: [0, -1], rank: 0 }, // UP
      { delta: [1, 0], rank: 1 },  // RIGHT
      { delta: [0, 1], rank: 2 },  // DOWN
      { delta: [-1, 0], rank: 3 }, // LEFT
    ];
    ...
    for (const { delta, rank } of cardinals) {
      ...
      // onward_open: candidate 的四邻中非障碍 + 非 Core 自身的开放数
      let onwardOpen = 0;
      let nearbyEnemies = 0;
