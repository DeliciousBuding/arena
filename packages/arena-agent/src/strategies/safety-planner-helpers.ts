...
export function guardHomeCell(
  ...
  // 疏散到外环——2-3 环仅 16 格，t1 生产实证 14 个单位挤核心 3 格（外环全占
  // → 回退 4 邻 → 核心被围死 → 清路/REPLAN 死循环）；4-5 环再提供 48 格
  // 疏散容量。非迁移期守卫目标仍优先 2-3 环（拱卫距离近，第 1-8 候选不变，
  // 历史行为零回归——测试断言 index 0 = [-2,-2] 保持）。
