...
export function buildMacroPolicyPrompt(
  ...
  return [
    ...
    "attackPriority 约束（模拟器对打 300 ticks 实证，aggressive 姿态下）：校准供给 cadence（≈65 tick/格，",
    "匹配真实服务器）下 core/workers/null 三目标资源差无显著区分（edge 3.5-4.0，workers 微优）；",
    "拆家掠夺（core）的大幅优势（旧实验 edge +33）只在对手供给枯竭时成立（未校准慢供给特例）——",
    "真实供给下攻击目标按战局选：对手经济健康选 workers 断其采集、对手经济已崩选 core 终结、",
    "无明确目标用 null；balanced/harvest 用 null（前压不触发，字段不消费）。",
    "",
    "宏观状态：",
    `tick: ${state.tick}`,
