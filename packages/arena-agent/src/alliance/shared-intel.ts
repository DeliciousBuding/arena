...
import type { AllianceMemberReport, EntitySighting, SightingEvidence } from "./types.ts";
...
const EVIDENCE_RANK: Readonly<Record<SightingEvidence, number>> = Object.freeze({
  LIVE: 4,
  CALIBRATION: 3,
  LEADERBOARD: 2,
  HISTORY: 1,
...
export function aggregateAllianceIntel(input: AggregateAllianceIntelInput): SharedIntelView {
  ...
  const filtered = (input.sightings ?? []).filter((s) => !allyIds.has(s.key));
