...
// ---------- 第三方 agent 注册/心跳（2026-08-09，SDK telemetry ingest） ----------
...
/** 将 SDK 上报的一条 telemetry 事件写入台账表（agents + agent_events）。 */
export function applyAgentEvent(db: DatabaseSync, event: AgentIngestEvent): void {
  ...
  ).run(event.tenant, instance, event.event, tick, detail, event.ts);
}
