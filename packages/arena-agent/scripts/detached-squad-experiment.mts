 ...
 * ③2 守卫：p1 2v2 集火逐个击破并拆掉 p2 Core（t14 CORE_DESTROYED）——
 * 变体错失进攻机会（负收益）；④3 守卫：交战仍发生（p1 仍集火占优）。
 * 注（2026-08-07 引擎验证修正）：战斗为快照同时结算（官方 combat.md
 * "Nothing grants initiative"——sweep-simultaneity-check 实证互砍
 * 同归于尽），无先手优势；p1 打赢真实机制 = 集火逐个击破（tick2-3
 * 双 SWEEP 集火单守卫，p2 仅 1 守卫可即时接敌、其余守位距 3-4 格
 * 无法 SWEEP）。"同速同血先手必胜"为旧表述误读，已修正：无法构造
 * "拦截力量占优"的稳定场景 → B5 净收益无一致证据，变体保持候选不启用。
