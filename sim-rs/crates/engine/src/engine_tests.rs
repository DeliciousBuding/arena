...
fn plan_with(actions: Vec<(String, UnitAction)>) -> Plan {
    Plan {
        tick: 1,
        unit_actions: actions.into_iter().collect(),
        core_action: None,
...
fn settle_blocks_obstacle() {
    ...
    assert!(result.events.iter().any(|e| e.event_type == "MOVE_BLOCKED"
        && e.reason_code.as_deref() == Some("MOVE_BLOCKED_OBSTACLE")));
}
...
fn settle_deterministic() {
    ...
    assert_eq!(
        first.next_state.units[0].position,
        second.next_state.units[0].position
    );
...
fn settle_full_cycle() {
    ...
    let harvest_plan = plan_with(vec![(
        "worker-1".to_string(),
        UnitAction {
            kind: UnitActionKind::Harvest,
            direction: None,
...
pub fn run_ticks(
    state: &mut TickState,
    ticks: i32,
    refill_cells: &[arena_sim_domain::Position],
) -> SettleStats {
