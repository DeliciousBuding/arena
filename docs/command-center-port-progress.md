...
### 9.31 资产列表编队成员高亮（2026-08-08）
- tactRenderAssets asset-row 追加 squad class（multi 命中且非主选中时，
...
### 9.34 小地图完整修复（2026-08-08，48e9609）
- 用户反馈右下角缩略图"白色乱七八糟"，逐项定位并根治：
  1. **白色蒙层**：整幅 `rgba(255,255,255,.03)` fill 在透明位图上产生 ~8000 个
     (255,255,255) 半透明白像素 → 删除（底由 CSS 深底透出）。
  2. **chunk 底纹错位**：区块是 16×16 世界格，旧实现把区块索引当世界坐标投影，
     295 个 chunk 全挤到中右一坨；改为 ×16 转世界坐标 + 真实像素尺寸 + 视口剔除。
  3. **视野白框**：全局/fitView 下视野≈全图，旧面积阈值仍整圈白框贴边 + 叠 CSS
     边框；改语义判定（视口已包含整个世界则不画框），正常缩放时钳到小地图边界 +
     框内轻提亮。
  4. **高 DPR 错位**：位图=CSS×dpr 但未 setTransform，dpr>1 时内容只画左上角四分之一；
     init+draw 均 setTransform，draw 内按当前 dpr 重同步位图（跨屏自适应）。
- 实测：dpr=1 全局 opaqueWhite 8003→0；dpr=2 位图 344×256 内容覆盖 94%（修复前只左上角）；
  缩放后视野框小框正确显示。typecheck 0 错 + build 通过 + 完整回归 22/22 全绿。
