...
- **启用租户**：`D:/Code/Projects/arena/data/runtime/configs/t{1,2,3,4}.json` 的
...
- 风险：在 main 工作树注册表上加载 t1/t2 的 variants 数组会 fail-fast
  ...
  `docs/design/project-org-plan-20260808.md`（基线收口 60/226 漂移 + 孤儿变体）。
...
> core-evade-v1 不算"无租户启用"（见 PROGRESS.md"实读发现"）。本表以实读为准。
