...
- [ ] C3: 全局阶段状态机（EARLY_EXPANSION → BALANCED → MILITARY）
  - 验收：转移规则（资源/人口/敌人威胁阈值）可配置；默认 EARLY 启动；转移单测
- [ ] C4: 单元状态机（Worker PATROL→GO_HARVEST→HARVEST→RETURN→DEPOSIT；战斗 CHASE/GUARD/FIRE）
  - 验收：Fake 序列测试覆盖完整转移链 + HARVEST_FAILED/RESOURCE_DEPLETED 重定向
- [ ] D1: `debug_api.py` HTTP 调试端点（127.0.0.1）
