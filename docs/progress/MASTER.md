...
- **工具**：`scripts/strategy-search.mts`（策略网格批量验证，v3：27 组合两阶段搜索）、`scripts/military-ratio-experiment.mts`（军事比例聚焦实验，结果文件落盘）。
- **策略空间搜索结论（v2/v3）**：workerTarget=10 单人经济最强（43.3 vs 30@6）但对打输给 6-worker 基准（upkeep 负担）；**8 worker 纯经济对打略胜 6-worker 基准**（31.5 vs 30、pop 8 vs 6）——**8 worker + militaryRatio 0.3 是平衡区**（6 保守、10 负担；0.5+ 纯损耗）。生产 t1（LLM 演进 7/0.5）与 t2（12/0.5 濒死）均可用此解释。结论强度：对打为准（单人 150 ticks 区分度弱，8/10 未拉开）。
- **本地生产约束生效（2026-08-06）**：发现本地 live 进程是迁移时旧代码（prompt 约束未生效，t1 漂移到 militaryRatio=0.8）→ 重启到最新代码（6bc6d52）后 **t1 立即收敛到 aggressive/8/0.4**（遵守拐点指引、workerTarget 平衡区）——**约束 prompt 生产生效实证**。t2 实验组从 12/0.5（濒死）调整为 balanced/8/0.3（平衡区验证）——**切换后 10+ 分钟未恢复**（res 0-5 波动、pop 2）：经济雪崩后真实对局无恢复窗口——**预防（约束）> 治疗（切换）**，强化 prompt 约束价值。**t2 终局（持续观察至 2026-08-06）**：balanced/8/0.3 长期停留 res 0-5/pop 2——雪崩不可逆结论坐实（真实对局无恢复窗口），t2 继续作为"不可恢复对照组"运行（校准 dataset 持续采集崩溃态样本）。
- **真实数据校准（2026-08-06，模拟器对真实对局回放）**：t1/t2 calibration dataset 各 408+ cases（before/after/plan/seed 对，rulesVersion v0.11 与本地 manifest 对齐）。修复 calibrate CLI 两个问题后跑 24 个真实 case：**全部 INCONCLUSIVE、零硬差异**（差异全为 EXPECTED_UNKNOWN：对手行动/信标状态/动态障碍视野噪音——真实对局中障碍集合每 tick 因视野变化 24→29±，模拟器静态记忆属诚实建模）。修复内容：canonical JSON reject undefined（canonicalize undefined→null + 差异值归一 + event.values ?? null）；动态对局中障碍差异归 EXPECTED_UNKNOWN（原 TERRAIN 硬差异误报）。**校准管道可用性实证：模拟器对真实数据无确定性误差**。
- **attackPriority 实证（2026-08-06，攻击目标对比实验）**：p1=balanced/8/0.3/null 基准 vs p2=aggressive/8/0.3 变 attackPriority（300 ticks × 2 seeds + refill，间距 10→4 格进视野——发现 **前压仅在 aggressive 触发、无可见敌人时军队不出击**，balanced 下 attackPriority 字段不消费）。结果：**core 拆家掠夺 edge +33.0 最优**（p1 pop 打到 2，拆家掠夺资源终结对手产兵）；null 居中 +32.5（p1 pop 6.5）；**workers 断经济最差 +19.0**（p1 pop 6 存活、Core 继续产兵）。**prompt 约束落地**：aggressive 优先 attackPriority=core、balanced/harvest 用 null。生产 t1（aggressive/8/0.4/attackPriority=null）下个周期应转向 core。
- **军事配比实证（2026-08-06，vanguardRatio 配比实验）**：SafetyPlanner/DeterministicPlanner 加 vanguardRatio（VANGUARD 目标占比，undefined=交替产兵零回归），EpisodeTenant.plannerConfig 透传（模拟器配比实验基础设施）。修复配比计算 bug（floor(military×ratio) 在 military=0 恒 0 → ceil((military+1)×ratio)）。**双场景结论（300 ticks × 2 seeds，p2=aggressive/8/0.3/core 变配比）**：对手防守（balanced）→ Vanguard 配比越高越优（全 Vanguard +35.0 / 交替 +33.0 / 全 Ranger +28.0，攻坚拆家无对攻压力）；**对手对攻（aggressive）→ 全 Ranger 唯一存活（+28.0 vs 含 Vanguard 全灭 -22.5，Vanguard 前压被集火送死）**。**配比应适配对手姿态，无全局最优固定配比；默认交替（0.5）是双场景稳健折中**——生产默认不改，vanguardRatio 留给策略层自适应。
- **测绘盲区修复链（2026-08-06，用户实证：t1 [-594,-169] 矿前端可见但算法不开采）**：矿距 Core 40 格（Core [-619,-154]）。**有测绘/视野/记忆逻辑**（world.ts 视野记忆 + resourceHints + 巡逻环），但三个参数让测绘效率极低：① 巡逻环只到 32 格（EXPLORE_RING_COUNT=4）→ 5 环覆盖 40 格；② 巡逻方位连续步进（+1）按 beacon 东南基逐圈推进，正东矿第 8 圈才轮到（数百 tick 不可达）+ worker 初始方向连续（首圈聚集 4 方位）→ 步进 +3（与 8 互质前 3 圈扫全 8 方位）+ 初始方向 (index×3)%8 错开；③ 记忆窗口 8 tick 太短（巡逻一圈数十 tick 刚见过就忘）+ 耗尽冷却 4 tick 太短（worker 反复试近处空矿、maxDist 12-17 空转、巡逻环永不推进）→ maxAge 8→32、failedCooldown 4→32 配对。**验证**（稀缺场景矿 40 格外、无 refill、400 ticks×2 seeds）：res 2→3、maxDist 17→71（balanced/harvest/aggressive 三姿态均生效）；focus 32 上限仍够不到 40 矿（maxFocusDistance 上限限制）；探索半径 24 仍乱跑无效。相关测试 35/35 通过。**结论：测绘不是没有，是覆盖效率参数错了——已修复并生产生效**（t1/t2 重启 29e7f69）。
