...
- **三阶段爆兵节奏（2026-08-06，用户导向"积累到一定程度开始爆兵，以爆兵为目的打对面水晶"）**：`accumulateThreshold` + `attackForce`（默认关闭零回归）。积累期（res<阈值只产 worker）→ 爆兵期（达标 `surgeActive` 状态保持、全力交替产兵直到资源耗尽回积累期——修复"产 1 兵掉回阈值下"振荡）→ 前压期（军事达 `attackForce` 才前压拆水晶，未成型守家蓄势不送死）。接线 deterministic 产兵路径（`selectDeterministicCoreAction` 返回 surgeActive 回写）。**验证**（对打 400 ticks×2 seeds+refill）：爆兵节奏 p2 军 3.5 成型、p1 pop 打残 2、edge +33（对比无节奏：1 军守家 edge 0）。`scripts/military-surge-experiment.mts`。
- **军事打野 + 连续外扩巡逻（2026-08-06，用户导向）**：① aggressive+无敌人+资源枯竭时 Vanguard 巡逻外扩（`vanguard_scavenge`——测绘+打野，不再守家发呆）；② 巡逻连续外扩——到达环点不回 home 换环，同方位直接延伸下一环（8→16→24→32→40，"一直往外探索"；旧行为每环往返视觉上来回、推进慢）；③ worker 初始方向 +7 偏置（beacon 东南 base 偏置导致正东矿首圈不可达的盲区修复）。
