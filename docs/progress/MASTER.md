...
- **资源枯竭应对实证（2026-08-06，focusRegion 维度补全 + t1 枯竭确诊）**：t1 生产确诊**资源枯竭终局**（visibleRes=0、res 冻结 4、pop 8 巡逻中、无 stall 事件——真实对局近处资源采完，官方 refill 不可知）。模拟器枯竭场景（近处 2 格 + 远处 30 格外 6 格、无 refill、400 ticks）：**balanced/null 守家 res 2 恒死；focusRegion 指向远矿（合理近距焦点）res 2→3、pop 4→5、worker 达 33 格——唯一有效应对**；探索半径 8→24 反而乱跑 111 格无效（res 2）；harvest 扩员无效。**prompt 枯竭告警落地**：visibleRes=0 时高亮警告（二选一：aggressive 前压抢资源 / focusRegion ≤30 格定向探索；禁止守家组合）。执行层探索半径保持 8（放大无收益）。
