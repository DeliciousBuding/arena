...
- **本地生产约束生效（2026-08-06）**：发现本地 live 进程是迁移时旧代码（prompt 约束未生效，t1 漂移到 militaryRatio=0.8）→ 重启到最新代码（6bc6d52）后 **t1 立即收敛到 aggressive/8/0.4**（遵守拐点指引、workerTarget 平衡区）——**约束 prompt 生产生效实证**。t2 实验组从 12/0.5（濒死）调整为 balanced/8/0.3（平衡区验证）——**切换后 10+ 分钟未恢复**（res 0-5 波动、pop 2）：经济雪崩后真实对局无恢复窗口——**预防（约束）> 治疗（切换）**，强化 prompt 约束价值。
